...
import { arr2 } from '@/json/datas-0925';
...
onMounted(() => {
	...
	let nTime = getTime()
	if(!!arr2[nTime]) {
		data.topArr = arr2[nTime]
	} else {
		if(day === 21) {
			data.topArr = Object.values(arr2)[20]
		} else if(day === 22) {
			data.topArr = Object.values(arr2)[30]
		} else {
			data.topArr = Object.values(arr2)[0]
		}
	}
    data.topArr = data.topArr.sort(compare('val'))
    TopRender2(data.topArr)
	setArr()
	// test()
})
...
const test = () => {
	let testArr:any = Object.values(arr2)
	setInterval(() => {
		data.inx++
...
const TopRender2 = (resData: TopArr) => {
